...
def test_tritone_component():
    ...
    N_burn = (
        9.3e-7 * AF
    )  # Tritium burn rate in the plasma adjusted for AF - THIS IS IMPACTING THE RESERVE INVENTORY
    TBR = 1.067
    tau_bb = 1.25 * 3600
    tau_fc = 3600
    tau_tes = L / U0
    ...
    final_time = 3600  # NB: longer than doubling time
    hx_to_fw = 0.33
    hx_to_div = 0.33
    hx_to_ds = 1e-4
    ...
    simulation = Simulate(
        dt=0.01, final_time=final_time, I_reserve=I_reserve, component_map=component_map
    )
    t, y = simulation.run()
